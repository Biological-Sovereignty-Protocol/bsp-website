...
Cinco módulos Move no Aptos aplicam as regras do protocolo. Uma vez implantados, são governados por autorização multi-sig.
...
4. **Verificação Zero-Trust:** O Relayer API obtém a chave pública do usuário do `BEORegistry` do Arweave. Verifica matematicamente a assinatura contra o payload.
   ...
   - **Assinatura Válida:** A API envolve a intenção exata do usuário em uma transação Aptos, paga o gas usando a carteira do Ambrósio Institute e a submete à blockchain Aptos. Dados biológicos associados são armazenados no Arweave.
