...
*(O diagrama acima mapeia o fluxo completo de dados: da submissão clínica nos contratos Aptos até o consumo do lado do cliente.)*
...
*   **Infraestrutura:** Contratos inteligentes Move no Aptos (execução/estado) + armazenamento permanente no Arweave (bio-dados).
...
*   **Responsabilidade:**
    ...
    *   Normalizar as transações Aptos que descrevem todas as interações do paciente nos blocos JSON em tempo real.
...
Contratos inteligentes Move no Aptos gerenciam identidades BEO, registros de domínio `.bsp` e permissões de acesso com finalidade sub-segundo — garantindo que as regras do protocolo não possam ser alteradas por nenhum ator único. O Arweave armazena os dados biológicos permanentemente.
